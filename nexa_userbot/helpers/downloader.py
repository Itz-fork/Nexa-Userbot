...
class Downloader:
    ...
    async def __isFinished(self, dl_obj):
        return dl_obj.isFinished()
    
    async def __isSuccess(self, dl_obj):
        return dl_obj.isSuccessful()
    
    async def __get_downloaded_file_details(self, dl_obj):
    ...
    async def __get_progress_details(self, dl_obj):
        details = {}
